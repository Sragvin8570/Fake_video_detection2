...
class MP_AViT(nn.Module):
    ...
    def forward(self, video, audio):
        ...
        # Make temporal embeddings robust to sequence length
        tv = self.temporal_visual_embedding
        if tv.size(2) < t_len:
            rep = (t_len + tv.size(2) - 1) // tv.size(2)
            tv = tv.repeat(1, 1, rep)[:, :, :t_len]
        else:
            tv = tv[:, :, :t_len]
        ta = self.temporal_audio_embedding
        if ta.size(2) < aud_t_len:
            rep_a = (aud_t_len + ta.size(2) - 1) // ta.size(2)
            ta = ta.repeat(1, 1, rep_a)[:, :, :aud_t_len]
        else:
            ta = ta[:, :, :aud_t_len]

        video = video + tv
        video = video + self.visual_modality_embeding
        audio = audio + ta
        audio = audio + self.audio_modality_embeding
        b, d, _ = video.size()
        video = video.reshape(b, -1, d) 
        #video = video.permute([0, 2, 1])
        audio = audio.reshape(b, -1, d)
